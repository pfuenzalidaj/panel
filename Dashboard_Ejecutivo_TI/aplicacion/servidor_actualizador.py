...
TEMP_DIR = PACKAGE_DIR / "temporales"
UI_FILE = APP_DIR / "interfaz_actualizador.html"
DASHBOARD_FILE = PACKAGE_DIR / "dashboard" / "Dashboard_Ejecutivo_Gestion_TI.html"
PROBLEMS_FILE = SOURCES_DIR / "Informe_Problemas.xlsx"
CHANGES_FILE = SOURCES_DIR / "Reporte_RFC-CAB_2026_237_278_202607101029.xlsx"
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route in ("/", "/index.html"):
            ...
            self.send_file(PACKAGE_DIR / route.lstrip("/"))
    ...
    def handle_update(self):
        ...
        changes_temporary_path = None
        try:
            with tempfile.NamedTemporaryFile(
                prefix="tickets_", suffix=".xlsx", dir=TEMP_DIR, delete=False
            ) as temporary:
            ...
            changes_path = CHANGES_FILE if CHANGES_FILE.is_file() else None
            changes_original_name = CHANGES_FILE.name if changes_path else None
            if changes_upload is not None and changes_upload.get_filename():
                changes_original_name = Path(changes_upload.get_filename()).name
                if Path(changes_original_name).suffix.lower() != ".xlsx":
                    raise ValueError(
                        "El archivo de cambios debe ser un archivo .xlsx."
                    )
                changes_bytes = changes_upload.get_payload(decode=True) or b""
                if not changes_bytes.startswith(b"PK"):
                    raise ValueError(
                        "El archivo de cambios no parece ser un XLSX válido."
                    )
                with tempfile.NamedTemporaryFile(
                    prefix="cambios_",
                    suffix=".xlsx",
                    dir=TEMP_DIR,
                    delete=False,
                ) as changes_temporary:
                    changes_temporary.write(changes_bytes)
                    changes_temporary_path = Path(changes_temporary.name)
                changes_path = changes_temporary_path

            generator = importlib.reload(generador_dashboard)
            data = generator.update_dashboard(
                ...
                changes_source_name=changes_original_name,
            )
            self.send_json(
                {
                    "ok": True,
                    ...
                    "changesSource": data.get("fuente_cambios", {}).get("archivo"),
                    "changes": data.get("change_kpi", {}).get("total"),
                    "changeSuccess": data.get("change_kpi", {}).get("success"),
                    "changeFailed": data.get("change_kpi", {}).get("failed"),
                    "dashboard": DASHBOARD_FILE.name,
                }
            )
            ...
            if changes_temporary_path and changes_temporary_path.exists():
                changes_temporary_path.unlink()
...
def main():
    ...
    for pattern in ("tickets_*.xlsx", "problemas_*.xlsx", "cambios_*.xlsx"):
